export default {
  async fetch(request, env, ctx) {
    ...
    if (!watermarkResp.ok || !watermarkResp.body)
      return new Response('Missing watermark', { status: 404 })
    ...
    const response = (
      await env.IMAGES.input(imageResp.body)
        .draw(env.IMAGES.input(watermarkResp.body).transform({ width: 32, height: 32 }), {
          bottom: 32,
          right: 32,
        })
